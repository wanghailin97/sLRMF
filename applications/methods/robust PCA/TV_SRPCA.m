function [ X ] = TV_SRPCA(M, beta, tau, rho, mu, max_mu, maxIter, tol)
%% Nuclear norm based stable RPCA (NN_SRPCA) via ADMM: 
%  min_{X} 1/2*||X+S-M||_F^2 + tau*||S||_1 + beta*||Dx(X)||_1 + beta*||Dy(X)||_1,
...
while iter < maxIter
    ...
    if iter == 1 || mod(iter, 10) == 0
        obj = 0.5*norm(X+S-M,'fro')^2 + tau*norm(S,1) + beta * (norm(G1(:),1) + norm(G2(:),1));
