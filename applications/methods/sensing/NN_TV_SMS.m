function [ X ] = NN_TV_SMS(y, A, dim, gamma, beta, rho, mu, max_mu, maxIter, tol)
%% Nuclear norm plus total variation based stable matrix sensing (NN_TV_SMS) via ADMM: 
%  min_{X} 1/2*||y-A(X)||_2^2 + gamma*||X||_* + beta*||Dx(X)||_1 + beta*||Dy(X)||_1,
...
% if nargin < 5; beta    = sigma / sqrt(min(m,n));          end 
if nargin < 6; rho     = 1.1;                             end     
if nargin < 7; mu      = 1e-4;                            end  
if nargin < 8; max_mu  = 1e10;                            end  
if nargin < 9; maxIter = 500;                             end          
...
while iter < maxIter
    ...
    B = Aty + enDiffT(mu*G1 - M1,1) +  enDiffT(mu*G2 - M2,2) + (mu*Z - M3);
    ...
    if iter == 1 || mod(iter, 10) == 0
        obj = 0.5*norm(y-A*X(:))^2 + gamma * nn_Z + beta * (norm(G1(:),1) + norm(G2(:),1));
