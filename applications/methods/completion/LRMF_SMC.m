...
%  min_{X,Y} 1/2p*||(X*Y')(Mask)-M(Mask)||_F^2 + lambda/2p*||X||_F^2 + lambda/2p*||Y||_F^2 + 1/8*||X'X-Y'*Y||_F^2,
%  parameters: 
...
while iter < maxIter
	...
	%% updates
    Grad_Xk = ((q*Xk*Yk').*Mask - qM_Mask) * Yk + q * lambda * Xk + 0.5 * Xk * (Xk'*Xk-Yk'*Yk);
    X = Xk - lr*Grad_Xk;
    Grad_Yk = ((q*Xk*Yk').*Mask - qM_Mask)' * Xk + q * lambda * Yk + 0.5 * Yk * (Yk'*Yk-Xk'*Xk);
    Y = Yk - lr*Grad_Yk;
