function [ M_est, S, X, Y ] = sLRMF_SRPCA_v1(M, rank, lambda, tau, zeta, lr, maxIter, tol)
%% Smoothed low-rank matrix factorization based stable RPCA (sLRMF_SRPCA) via gradient descent: 
%  min_{X,Y} 1/2*||X*Y'+S-M||_F^2 + tau*||S||_1 lambda/2*||D(X)||_F^2 + lambda/2*||D(Y)||_F^2 + 1/8*||X'X-Y'*Y||_F^2,
...
%     zeta   : trimming thresholding parameter
...
while iter < maxIter
	...
	%% gradient descent
    Grad_Xk = Rk  * Yk + lambda * DiffT(Diff(Xk)) + 0.5 * Xk * Bk;
    X = Xk - lr*Grad_Xk;
    Grad_Yk = Rk' * Xk + lambda * DiffT(Diff(Yk)) - 0.5 * Yk * Bk;
    Y = Yk - lr*Grad_Yk;
